...
fn main() -> Result<(), String> {
    ...
    let mut controller1 = (1..available)
        .find_map(|id| {
            if !game_controller_subsystem.is_game_controller(id) {
                println!("{} is not a game controller1", id);
                return None;
            }

            println!("Attempting to open controller1 {}", id);
    ...
    println!("Controller1 mapping: {}", controller1.mapping());
    ...
    for event in sdl_context.event_pump()?.wait_iter() {
        ...
        match event {
            ...
            Event::ControllerButtonDown { which, button, .. } => {
                println!("{} - Button {:?} down", which, button)
            }
