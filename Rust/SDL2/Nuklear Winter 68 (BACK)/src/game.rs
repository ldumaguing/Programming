...
pub fn update(joystick_manager: &mut u16, gv: &mut GlobalVariables) {
    ...
    // ***** define hex_id
    let hexagon_top_left: (f64, f64) = ((gv.hex_0x0.0 - gv.hexagon_half.0), (gv.hex_0x0.1 - gv.hexagon_half.1));
    println!("hexagon_top_left({}, {})", hexagon_top_left.0, hexagon_top_left.1);
    let mut x = ((gv.cursor_loc.0 + gv.map_loc.0 - hexagon_top_left.0) / gv.hexagon.0) as i32;
    let mut y: i32 = 0;
    if (x % 2) == 0 { // ***** even X
        y = ((gv.cursor_loc.1 + gv.map_loc.1 - hexagon_top_left.1) / gv.hexagon.1) as i32;
    } else { // ***** odd X
    ...
    println!("cursor_loc_real({},{}): ", gv.cursor_loc_real.0, gv.cursor_loc_real.1);
    println!("hexagon_half({},{}): ", gv.hexagon_half.0, gv.hexagon_half.1);
    println!();
}
