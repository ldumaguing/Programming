...
void GFX_drawImage(int x, int y, int w, int h, int16_t *image) {
	/*
	int counter = 0;
	for(int j=0+y; j<(h+y); j++) {
		for(int i=0+x; i<(w+x); i++) {
			screen_buffer[i + j*240] = image[counter];
			counter++; 
		}
	}
	*/
...
/*
	for(int X=0; X<w; X++) {
		for(int Y=0; Y<h; Y++) {
			GFX_drawPixel(X, Y, image[X+(Y*w)]);
		}
	}
*/
	
/*
	for(int X=0+x; X<(w+x); X++) {
		for(int Y=0; Y<h; Y++) {
			GFX_drawPixel(X, Y, image[(X-x)+(Y*w)]);
		}
	}
*/
	...
	for(int X=0+x; X<(w+x); X++) {
		for(int Y=0+y; Y<(h+y); Y++) {
			// GFX_drawPixel(X, Y, image[(X-x)+(Y*w)]);
			GFX_drawPixel(X, Y, image[(X-x)+((Y-y)*w)]);
		}
	}
	...
	for(int X=0+x; X<(w+x); X++) {
		for(int Y=0+y; Y<(h+y); Y++) {
			GFX_drawPixel(X, Y, image[X+((Y-y)*w)]);
		}
	}
	*/
}
...
void GFX_fillScreen2(uint16_t color1, uint16_t color2) {
	int i = 0;

	while (i < ILI9341_SIZE) {
		screen_buffer[i] = color1;
		i++;
	}
	
	for(int x=ILI9341_SIZE/2; x<ILI9341_SIZE; x++) {
		screen_buffer[x] = color2;
	}
}
