...
static int getInt(uint8_t *str, int loc) {
	uint8_t buff[16];
	int count=0;
...
int UART_getDPad(void) {
    return padinputs.dpad;
}
...
static void rx_task(void *arg) {
    ...
    while (1) {
        const int rxBytes = uart_read_bytes(UART_NUM_x, data, RX_BUF_SIZE, 20 / portTICK_PERIOD_MS);
        if (rxBytes > 0) {
            data[rxBytes] = 0;
