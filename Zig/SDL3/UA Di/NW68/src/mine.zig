...
pub fn AppIterate() void {
     _ = c.SDL_BlitSurface(m.boardgame_surface, null, m.window_surface, null);
 
    _ = c.SDL_SetRenderDrawColor(m.renderer, 255, 255, 255, c.SDL_ALPHA_OPAQUE);
    _ = c.SDL_RenderLine(m.renderer, 0, 0, m.WINDOW_WIDTH, m.WINDOW_HEIGHT);

    _ = c.SDL_RenderPresent(m.renderer);

 
     // _ = c.SDL_UpdateWindowSurface(m.window);
