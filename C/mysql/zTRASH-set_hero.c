...
void calc_Swing(char*, MYSQL*);
void Swing_1(int, MYSQL*, char*);
void Swing_2(int, MYSQL*, char*);
void Swing_3(int, MYSQL*, char*);
void update_Swing(char*, char*, MYSQL*);
...
// =======================================================================================
void calc_Swing(char* id, MYSQL* conn) {
	char stmt[512];
	sprintf(stmt, "SELECT"
...
// ---------------------------------------------------------------------------------------
void Swing_3(int val, MYSQL* conn, char* id) {
	char dice[80];
	...
	if (val == 25)
		modifier = -1;
	if (modifier < 0) {
		sprintf(dice, "5d%d", modifier);
		update_Swing(dice, id, conn);
		return; }
	if (modifier == 0) {
		sprintf(dice, "5d");
		update_Swing(dice, id, conn);
...
// ---------------------------------------------------------------------------------------
void Swing_2(int val, MYSQL* conn, char* id) {
	char dice[80];
...
// ---------------------------------------------------------------------------------------
void Swing_1(int val, MYSQL* conn, char* id) {
	char dice[80];
	int modifier = 0;

	if ((val >= 1) & (val <=9)) {
		modifier = round(val / 2.0) - 6;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void update_Swing(char* dice, char* id, MYSQL* conn) {
	char stmt[512];
	sprintf(stmt, "UPDATE TheWorld"
		" SET Definition = JSON_REPLACE(Definition,"
		" '$.\"secondary characteristics\".\"damage\".\"swing\"',"
		" \"%s\")"
		" where Id = %s", dice, id);
	if (mysql_query(conn, stmt))
		puts("error thrust"); }
