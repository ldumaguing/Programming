...
// *******************************************************************************
int Roll_For_Damage(char *dmgString) {
   // puts("----");
   char left[80];
   getGrep(left, "^[0-9]*", dmgString);
   // puts(left);

   char right[80];
   getGrep(right, "[0-9cutpim\\ +-]*$", dmgString);
   getGrep(right, "[0-9\\ +-]*", right);
   // puts(right);

   char Rleft[80];
   getGrep(Rleft, "^[0-9+-]*", right);
   // puts(Rleft);

   char Rright[80];
   getGrep(Rright, "\\ [0-9+-]*", right);
   // puts(Rright);
   ...
   int diceroll = roll_d6_X_times(atoi(left));

   // printf("diceroll: %d\n", diceroll);


   return diceroll + totalRight;
}
